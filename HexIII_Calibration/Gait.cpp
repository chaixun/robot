        ...
        if(isReadytoSetGait[i]==true)
        {
            ...
            case GAIT_AUTO_CALIBRATION:
                if(p_gait[i]!=m_currentGait[i])
                {
                    ...
                    m_currentGait[i]=p_gait[i];
                    m_gaitStartTime[i]=p_data.time;
                    m_commandDataMapped[motorID].Position=GaitAutoCalib[0][motorID];

                    rt_printf("driver %d:Begin GAIT_AUTO_CALIBRATION...\n",i);
                }
                else
                {
                    m_gaitCurrentIndex[i]=(int)(p_data.time-m_gaitStartTime[i]);
                    m_commandDataMapped[motorID].Position=GaitAutoCalib[m_gaitCurrentIndex[i]][motorID];

                    if(m_gaitCurrentIndex[i]==GAIT_AUTOCALIB_LEN-1)
                    {
                        rt_printf("driver %d: GAIT_AUTO_CALIBRATION will transfer to GAIT_STANDSTILL...\n",i);
                        ...
                        if(IsAutoCalibrateRegistered == true)
                        {
